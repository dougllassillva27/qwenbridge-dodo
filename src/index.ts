...
// [Dodo] Aumenta exponencialmente o timeout nativo do Node.js (undici) para 2 horas
// Previne o erro UND_ERR_HEADERS_TIMEOUT em prompts massivos (ex: /compact com 1M de tokens)
...
try {
  import('undici').then(({ setGlobalDispatcher, Agent }) => {
    setGlobalDispatcher(new Agent({
      headersTimeout: 7_200_000, // 2 horas
      bodyTimeout: 7_200_000,    // 2 horas
      connectTimeout: 60_000,    // 1 minuto
    }));
  }).catch(() => {});
} catch {}
