...
export async function solveBaxiaCaptcha(page: Page, accountId: string): Promise<boolean> {
  ...
  const sliderSelector = '#nc_1_n1z, .btn_slide, #aliyunCaptcha-sliding-slider';
  const wrapperSelector = '#nc_1_wrapper, .nc_wrapper, .baxia-punish, #WAF_NC_WRAPPER, #aliyunCaptcha-window-embed';
  const trackSelector = '#nc_1_n1t, .nc_scale, #aliyunCaptcha-sliding-body';
  ...
  if (foundMain) {
    Logger.captcha(`[CaptchaResolve] Baxia captcha detected on main document for ${accountId}.`);
  } else if (foundIframe) {
    Logger.captcha(`[CaptchaResolve] Baxia captcha iframe detected for ${accountId}.`);
  } else {
    Logger.captcha(`[CaptchaResolve] No VISIBLE captcha found for ${accountId} despite trigger.`);
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      const isPuzzle = await locatorContext.locator('#aliyunCaptcha-puzzle, .puzzle').first().isVisible().catch(() => false);

      // Tentativas 1 e 2: Abordagem Matemática Rápida e Gratuita
      if (attempt <= 2 && !isPuzzle) {
        Logger.captcha(`[CaptchaResolve] Attempt ${attempt}: Using fast math calculation for distance...`);
      ...
      // Tentativa 3: Fallback usando Microserviço ChatGPT Vision do usuário
        ...
        Logger.captcha(`[CaptchaResolve] Attempt ${attempt}: Using Vision-based captchaResolve (Microservice)...`);
        ...
        Logger.captcha(`[CaptchaResolve] Microservice returned x=${dragDistance}.`);
      }

      Logger.captcha(`[CaptchaResolve] Simulating drag for ${dragDistance}px...`);
      ...
      if (isWrapperGone || isSliderGone) {
        Logger.captcha(`[CaptchaResolve] Captcha solved successfully for ${accountId} (wrapper or slider disappeared).`);
        return true;
      }

      const okElement = locatorContext.locator('.btn_ok, .nc_ok, div#nc-loading-circle').first();
      if (await okElement.isVisible().catch(() => false)) {
        Logger.captcha(`[CaptchaResolve] Captcha solved successfully for ${accountId} (OK state detected).`);
        await sleep(1500);
        return true;
      }

      Logger.warn(`[CaptchaResolve] Attempt ${attempt} failed. Retrying...`);
      await sleep(1000);
    } catch (err: any) {
      Logger.error(`[CaptchaResolve] Error during attempt ${attempt}:`, err.message);
      await sleep(1000);
    }
  }

  Logger.error(`[CaptchaResolve] Failed to solve after 3 attempts.`);
