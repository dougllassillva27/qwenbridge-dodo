...
export class StorageView implements TuiView {
  ...
  public async handleKey(key: KeyEvent): Promise<boolean | void> {
    ...
    if (key.name === "click" && key.mouse) {
      ...
      if (col >= 2 && col <= leftW - 1) {
        if (row === 13) {
          await this.handleKey({ name: "p", ctrl: false, shift: false, meta: false });
          return true;
        }
        if (row === 14) {
        ...
        if (row === 17) {
          await this.handleKey({ name: "r", ctrl: false, shift: false, meta: false });
          return true;
        }
      }
    }
  ...
  public render(width: number, height: number): string[] {
    ...
    if (this.profileStats.length === 0) {
      ...
      const maxProfiles = Math.max(2, Math.floor((contentH - 10) / 2));
      const visibleProfiles = this.profileStats.slice(0, maxProfiles);
      ...
      if (this.profileStats.length > maxProfiles) {
        rightContent.push(
          `  ${theme.dim(`... e mais ${this.profileStats.length - maxProfiles} perfil(is)`)}`,
        );
      }
    }
