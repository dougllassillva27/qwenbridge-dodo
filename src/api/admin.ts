...
import { addAccount, removeAccount, listAccounts } from '../core/accounts.ts';
import {
...
import { getUserActiveStreams } from '../core/user-manager.ts';
import { getStreamRegistry, abortStream } from '../core/stream-registry.ts';
import { getRecentToolCalls } from '../core/tool-call-debug.ts';
import { getAllSeries } from '../core/time-series.ts';
import {
  readEnvFile,
  persistEnvPatch,
  restartServer,
...
function getWarmPoolStats(): Record<string, number> {
  return {};
}

function getAccountActiveLoad(accountId: string): number {
  let count = 0;
  for (const s of getStreamRegistry().values()) {
    if (s.accountId === accountId) count++;
  }
  return count;
}

function getInUseAccounts(): string[] {
  const inUse = new Set<string>();
  for (const s of getStreamRegistry().values()) {
    if (s.accountId) inUse.add(s.accountId);
  }
  return Array.from(inUse);
}

function isAccountReady(accountId: string): boolean {
  return !isAccountOnCooldown(accountId);
}

function getReadyAccountCount(): number {
  return listAccounts().filter((a) => !isAccountOnCooldown(a.id)).length;
}
...
function signSession(expiresAt: number): string {
  ...
  const sig = crypto
    .createHmac('sha256', `qwenproxy:${adminPassword()}`)
    .update(payload)
    .digest('base64url');
  return `${payload}.${sig}`;
}
...
async function adminGuard(c: any, next: any) {
  ...
  if (!enabled) {
    return c.json(
      { error: 'Admin dashboard disabled. Set ADMIN_PASSWORD (or API_KEY) in .env.' },
      503,
    );
  }
  if (!verifySession(c)) {
...
adminApp.post('/api/login', async (c) => {
  ...
  if (!expected) {
    return c.json(
      { error: 'Dashboard admin desabilitado. Configure ADMIN_PASSWORD no .env.' },
      503,
    );
  }
...
adminApp.get('/api/session', (c) => {
  const enabled = adminPassword() !== '';
  if (!enabled)
    return c.json({ authenticated: false, enabled: false, reason: 'ADMIN_PASSWORD not set' });
  return c.json({
...
async function buildOverview(): Promise<any> {
  ...
  const accounts = listAccounts().map((a) => ({
    ...
    activeLoad: streamCounts.get(a.id) || 0,
  ...
  const memoryPct =
    systemTotal > 0 ? Number(((mem.rss / systemTotal) * 100).toFixed(1)) : 0;
  ...
  return {
    ...
    requests5xx,
    requestsSuccessRate:
      requestsTotal > 0 ? Math.max(0, (1 - errors / requestsTotal) * 100) : 100,
    latency:
      latency && typeof latency === 'object'
        ? { sum: latency.sum, count: latency.count }
        : undefined,
    latencyCompletion:
      latencyCompletion && typeof latencyCompletion === 'object'
    ...
    maxStreamsPerAccount: (config.accounts as any)?.maxStreamsPerAccount || 4,
    streamSlotWaitMs: (config.accounts as any)?.streamSlotWaitMs || 18000,
    readyAccountCount: accounts.filter((a) => a.ready).length,
...
adminApp.get('/api/metrics/export', adminGuard, (c) => {
  return c.body(metrics.formatPrometheus(), 200, {
    ...
    'Content-Disposition': 'attachment; filename="qwenbridge-metrics.txt"',
  });
});
...
adminApp.get('/api/users', adminGuard, (c) => {
  const userInfo = listUsers().map((u: UserRow) => ({
    id: u.id,
    email: u.email,
    apiKey: u.api_key,
...
adminApp.get('/api/settings', adminGuard, (c) => {
  ...
  return c.json({
    settings: safe,
    types,
    live: getLiveSettings(),
    liveKeys: Array.from(LIVE_KEYS),
    allowlist: Array.from(SETTINGS_ALLOWLIST),
    defaults: {
      port: config.server.port,
      host: config.server.host,
      headless: config.browser.headless,
      warmPoolSize: config.warmPool.size,
      warmPoolLowWater: config.warmPool.lowWater,
      warmPoolTtlMs: config.warmPool.ttlMs,
      accountLanes: config.accounts.lanes,
      accountMaxConcurrentStreams: config.concurrency.maxStreamsPerAccount,
      accountStreamSlotWaitMs: config.concurrency.busyWaitMs,
      headersTtlMs: 60000,
      backgroundHeaderRefresh: true,
      hybridSessionsEnabled: config.hybridSessions.enabled,
      userRateLimitRpm: config.users.defaultRateLimitRpm,
      userMaxConcurrency: config.users.defaultMaxConcurrency,
    },
  });
...
adminApp.post('/api/settings', adminGuard, async (c) => {
  const body: any = await c.req.json().catch(() => null);
  const patch = (body?.settings || {}) as Record<string, string | null>;
  try {
    const applied = persistEnvPatch(patch);
    const liveApplied: string[] = [];
    for (const [key, value] of Object.entries(patch)) {
      if (LIVE_KEYS.has(key)) {
        applyRuntimeSetting(key, value);
        liveApplied.push(key);
      }
    }
    return c.json({
      ok: true,
      applied,
      liveApplied,
      restartRequired: applied.some((k) => !LIVE_KEYS.has(k)),
    });
  } catch (err: any) {
    return c.json({ error: err.message }, 400);
  }
...
adminApp.get('/api/usage', adminGuard, (c) => {
  ...
  return c.json({
    users: getTopUsers(topLimit).map((u) => {
      const user = users.find((usr: UserRow) => usr.id === u.userId);
      return { ...u, email: user?.email || u.userId };
    }),
    models: getModelUsage(),
  });
...
adminApp.post('/api/test-chat', adminGuard, async (c) => {
  ...
  const json = await res.json().catch(() => ({ error: 'invalid json from upstream' }));
...
adminApp.get('*', (c) => {
  ...
  if (!file) {
    return c.html(
      `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>QwenBridge · Admin</title></head>` +
