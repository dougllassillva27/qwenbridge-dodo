...
test("config exposes only Playwright/thread-native current auth and context settings", () => {
  ...
  // Warm-context lifecycle: 1 warm context by default (minimized memory footprint)
  // + 30s idle TTL for overflow contexts.
  assert.equal(config.playwright.maxActiveContexts, 1);
  assert.equal(config.playwright.idleContextTtlMs, 30_000);
  ...
  // Per-turn tool-call cap: safe default 6 ensures multi-file generations do not
  // exceed Qwen Web output token limits and get truncated mid-call. Env-tunable
  // via MAX_TOOL_CALLS_PER_TURN; 0 disables the cap.
  assert.equal(config.retry.maxToolCallsPerTurn, 6);
});
