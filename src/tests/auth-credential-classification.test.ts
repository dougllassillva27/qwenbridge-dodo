...
test("masked password ('***') is intercepted and never treated as actual password", async () => {
  const { addAccount, loadAccounts, getAccountCredentials, removeAccount, invalidateAccountsCache } = await import("../core/accounts.ts");
  const testId = "test-mask-guard-account";
  const prevEnv = process.env.QWEN_ACCOUNTS;
  try {
    ...
    if (prevEnv !== undefined) {
      process.env.QWEN_ACCOUNTS = prevEnv;
    }
    invalidateAccountsCache();
  }
});
